"""Tests for credential and workspace resolution."""
...
class TestResolveCredentials:
    def test_flag_takes_priority(self, monkeypatch):
        ...
        result = resolve_credentials(token_flag="flag_key", workspace="acme.fulfil.io")
        assert result.api_key == "flag_key"
        assert result.method == "api_key"

    def test_env_var(self, monkeypatch):
        monkeypatch.setenv("FULFIL_API_KEY", "env_key")
        result = resolve_credentials(workspace="acme.fulfil.io")
...
class TestNormalizeWorkspace:
    def test_slug(self):
        assert normalize_workspace("acme") == "acme.fulfil.io"

    def test_full_domain_fulfil_io(self):
        assert normalize_workspace("acme.fulfil.io") == "acme.fulfil.io"

    def test_full_domain_fulfil_app(self):
        assert normalize_workspace("acme.fulfil.app") == "acme.fulfil.app"
